...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+Arabic:wght@400;600;700;900&display=swap'); *{box-sizing:border-box;margin:0;padding:0} @keyframes fadeUp{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}} @keyframes expandIn{from{opacity:0;transform:translateY(-5px)}to{opacity:1;transform:translateY(0)}} @keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.25;transform:scale(.55)}} @keyframes shimmer{from{opacity:.35}to{opacity:.75}} @keyframes ticker{from{transform:translateX(0)}to{transform:translateX(-50%)}} @keyframes glow{0%,100%{text-shadow:0 0 16px rgba(192,57,43,.4)}50%{text-shadow:0 0 32px rgba(192,57,43,.85)}} @keyframes spin{from{transform:rotate(0)}to{transform:rotate(360deg)}} .tab:hover{background:rgba(255,255,255,.06)!important;color:#aaa!important} .catbtn:hover{filter:brightness(1.35)!important} .refbtn:hover{background:rgba(192,57,43,.32)!important} .chcard:hover{filter:brightness(1.1)} .news-grid,.vid-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px} @media(max-width:640px){ .live-layout{grid-template-columns:1fr!important} .news-grid,.vid-grid{grid-template-columns:1fr!important} } ::-webkit-scrollbar{width:5px} ::-webkit-scrollbar-track{background:#0d0d0d} ::-webkit-scrollbar-thumb{background:#2a2a2a;border-radius:3px}`}</style>
    ...
    {tab === "live" && (
      <div className="live-layout" style={{ display:"grid", gridTemplateColumns:"1fr 290px", gap:"16px", alignItems:"start" }}>
        ...
        <div style={{ background:"#0d0d0d", borderRadius:"16px", overflow:"hidden", border:"1px solid rgba(255,255,255,.07)" }}>
          {/* Header */}
          <div style={{ padding:"10px 14px", background:"#0f0f0f", borderBottom:"1px solid rgba(255,255,255,.05)", display:"flex", alignItems:"center", gap:"9px", flexWrap:"wrap" }}>
            <span style={{ width:8, height:8, borderRadius:"50%", background:"#e74c3c", display:"inline-block", animation:"pulse 1s infinite", flexShrink:0 }} />
            <span style={{ color:"#e74c3c", fontWeight:"800", fontSize:"12px", letterSpacing:"1.5px" }}>● بث مباشر</span>
            <span style={{ color:"#555", fontSize:"13px" }}>{liveCh.flag} {liveCh.name}</span>
            {/* زر يوتيوب مباشر */}
            <a
              href={`https://www.youtube.com/watch?v=${liveCh.youtubeId}`}
              target="_blank"
              rel="noopener noreferrer"
              style={{ marginRight:"auto", background:"#cc0000", color:"#fff", borderRadius:"7px", padding:"5px 12px", fontSize:"11px", fontWeight:"700", textDecoration:"none", display:"flex", alignItems:"center", gap:"5px", transition:"opacity .2s" }}
            >
              ▶ فتح في YouTube
            </a>
          </div>
          ...
          {/* Fallback bar */}
          <div style={{ padding:"11px 14px", background:"#0a0a0a", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"10px", flexWrap:"wrap" }}>
            <span style={{ color:"#333", fontSize:"11px" }}>💡 لا يعمل البث داخل الموقع؟</span>
            <a
              href={`https://www.youtube.com/watch?v=${liveCh.youtubeId}`}
              target="_blank"
              rel="noopener noreferrer"
              style={{ background:"rgba(204,0,0,.15)", border:"1px solid rgba(204,0,0,.4)", color:"#ff4444", borderRadius:"7px", padding:"5px 14px", fontSize:"12px", fontWeight:"700", textDecoration:"none", display:"inline-flex", alignItems:"center", gap:"6px" }}
            >
              🔴 شاهد مباشرة على YouTube
            </a>
          </div>
        </div>
